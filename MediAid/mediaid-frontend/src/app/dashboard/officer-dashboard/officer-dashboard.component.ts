...
export class OfficerDashboardComponent implements OnInit, OnDestroy {
  ...
  refreshCards() {
    this.summaryCards = [
      { label: 'Pending Enrollments', value: this.pendingEnrollments, icon: '⏳' },
      { label: 'Pending Claims', value: this.pendingClaims, icon: '⧗' },
      { label: 'Total Enrollments', value: this.totalEnrollments, icon: '☰' },
      { label: 'Active Disbursements', value: this.activeDisbursements, icon: '₹' },
    ];
  }
}
